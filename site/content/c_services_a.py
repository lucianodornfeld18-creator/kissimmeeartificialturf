...
def residential():
    body = "".join([
        sec("What is artificial grass installation in Kissimmee, FL?",
            ...
            + f"<p>Around here that's a different job than the same words mean in Arizona. Our base has to shed a Central Florida downpour, not just sit dry for ten months a year, and the fine sand under most Osceola County lots holds water close to the surface for weeks after a wet summer. {post('base-under-artificial-turf-florida-sandy-soil', 'What goes under turf on this sand')} covers the build in more depth than this page does.</p>"),

        sec("Who this fits, and who should think twice",
            "<p>Synthetic lawn suits yards where sod already struggles: heavy shade from a two-story house or a stand of live oaks, a side yard too narrow for a mower, a rental property nobody's around to water, or a homeowner tired of a chinch-bug fight every June. Replace grass with turf and the watering schedule, the mowing bill and the brown August patches all go away at once.</p>"
        ...
        sec("How do you pick the best artificial grass contractor near you?",
            "<p>You can't judge the work on the day it's finished; a good base and a shortcut base look the same until the first real storm. Judge the bid instead. Ask for the base depth and material in writing, whether seams are taped and glued or just nailed, where the water is supposed to go once the yard is covered, and the product's pile height and face weight by name, not \"premium turf.\"</p>"
    ...
    faqs = [
        ...
        faq("Does artificial turf attract more heat to the house?", "Not measurably more than a driveway or patio would. The surface itself gets hot in direct sun, covered above, but it doesn't raise indoor temperature the way a nearby low-E window reflection can raise the turf's own surface temperature."),
...
def putting():
    body = "".join([
        sec("What is backyard putting green installation in Kissimmee, FL?",
            ...
            + f"<p>Most residential greens in Osceola and Orange County run 300 to 800 sq ft, tucked into a back corner or wrapped around an existing pool deck. A simple flat pad with one cup is a weekend-scale project; a multi-tier green with breaks and a chipping pad takes longer to shape and costs more per foot. {svc('residential', 'A surrounding lawn')} usually gets built at the same time so the two surfaces meet cleanly.</p>"),
        ...
        sec("Putting green turf: nylon vs. polypropylene, and what stimp speed?",
            ...
            + f"<p>{a('/compare/nylon-vs-polyethylene-vs-polypropylene-turf/', 'The full fiber comparison')} and {post('artificial-turf-glossary', 'the turf glossary')} define stimp, nap and the other terms that show up on a putting green quote.</p>"),
...
def playground():
    body = "".join([
        sec("What is playground turf installation?",
            f"<p>Playground turf installation is a synthetic play surface built over a cushioned shock pad sized to the equipment's fall height, rather than a lawn-grade turf laid straight over dirt. It runs {price('playground')} per square foot as of {PRICE_DATE}: small residential play areas typically land at $10 to $16, and a daycare- or HOA-grade system with a full-thickness pad runs $15 to $25. In Kissimmee and Osceola County most projects take two to four days, and the bulk of that time goes into the base and pad, not the turf on top.</p>"
        ...
        sec("Where playground turf makes sense",
            "<p>It fits any play area under swings, slides or a climbing structure where mulch has to be raked back into place weekly, or where sand tracks into the house and holds standing water after a storm. It also suits an HOA common-area tot lot or a home daycare, where a documented, tested surface matters more than it does for a single backyard swing set.</p>"
        ...
        sec("What separates the best playground turf contractor from a lawn installer?",
            "<p>Ask for the shock pad's actual ASTM F1292 test documentation matched to the specific equipment's fall height, not a general product brochure. Ask whether the containment edge and drainage were designed before the pad went in or added afterward, since a play surface built around drainage from day one behaves differently than one where drainage was an afterthought. A contractor who can't produce fall-height documentation for the pad they're proposing isn't equipped for this particular job, even if they install excellent residential lawns.</p>"
