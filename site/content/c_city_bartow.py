...
HUB = page(
    ...
    capsule(f"Bartow is the Polk County seat, and a lawn conversion here answers to the city's own Building Department, not the county office two miles down the road. Installed turf still runs {price('residential')} a square foot as of September 2026, the same published range as the rest of Central Florida; what changes locally is water, permitting and a Peace River that begins just northeast of downtown."),
    "".join([
        sec("The county seat, and what that means for a turf crew",
            f"<p>Bartow is the Polk County seat, home to the county courthouse and, confusingly for a homeowner searching for the right office, its own city government layered on top of the county one. A turf job at a Bartow address goes through the city's own departments, not the Polk County Building Division. {ext(POP[1], 'Just over 20,000 people')} live inside the city limits, a mix of century-old homes near the courthouse square and newer rooftops on annexed land toward the edges of town. At {CITIES['bartow']['miles']} miles from downtown Kissimmee, Bartow sits at the far edge of where we work, and a job here typically shares a route with a stop in {city('lakeland')} or {city('winter-haven')}.</p>"),
        sec("Permits: Bartow's own Building Department",
            f"<p>The City of Bartow's Building Department, at 450 N. Wilson Avenue, takes permit calls at {ext(BUILD[1], '(863) 534-0157')}, answered in person or by voicemail around the clock, with applications filed through the city's own online portal rather than the county's Accela system. Bartow's own code stays quiet on synthetic turf specifically, at least in what's published, which makes {a('/laws/florida-hb-683/', "the state rule effective May 2026")} the most concrete standard a Bartow lot actually answers to. Confirming anything beyond that still means a call to the Building Department with the address in hand. A property just outside the city limit may sit in unincorporated Polk County instead, where a look at {a('/laws/permits/polk-county/', "Polk County's permit rules")} and a quick {ext(PA[1], 'parcel lookup')} tell you which building department to call. Where a homeowners' association sits on top, {a('/laws/hoa-rules/', 'state law still limits what it can restrict')} once turf sits behind a fence line.</p>"),
        ...
        sec("Water, Lake Hancock and the setback that follows the river",
            f"<p>Bartow bills its own water rather than buying through the county, and like the rest of the region it currently sits under the Southwest Florida Water Management District's Modified Phase III order. That order leaves a Bartow property with a single watering day, Monday through Friday depending on the last digit of the address, inside a pre-4-a.m. or post-8-p.m. window on anything smaller than an acre. {ext(WATER[1], "The city's water-restrictions page")} carries the current schedule, since an order like this one can change with little notice. {ext(HANCOCK[1], 'Lake Hancock')}, a 4,584-acre lake sitting mostly inside Bartow, marks the start of the Peace River at the point where {ext(RIVER[1], 'Saddle Creek and the Peace Creek Drainage Canal meet')} northeast of downtown. Any synthetic lawn near that shoreline, or along the river itself, holds to the state's usual 10-foot buffer, waived only where a seawall or bulkhead is already doing that job.</p>"),
        sec("Reclaimed ground and the oaks around the square",
            f"<p>South and east of downtown, some of Bartow's newer growth sits on ground that spent decades as an open phosphate pit before {ext(PHOSPHATE[1], 'state rules required it reclaimed')}, a standard in place for every mine permitted after 1975. Reclaimed land here has gone on to become pasture, fishing lakes and, on annexed parcels, home sites, but the soil under it is rebuilt rather than a natural Polk County sand series, and how consistently it compacts can vary block to block in a way the standard soil maps don't capture. Inside {ext(DOWNTOWN[1], 'the historic district ringing the courthouse square')}, the concern runs the other way: oaks shading Broadway and Summerlin since the early 1900s put {a('/laws/florida-hb-683/', "the state's drip-line rule")} into play on almost any lot old enough to sit in the district.</p>"),
        "<!--AUTO:city-services-->",
    ]),
    faqs=[
        ...
        faq("What's Bartow's current watering schedule?",
            "Once a week, on whichever weekday the last digit of the address lines up with, inside the early-morning or late-night hours SWFWMD's current order allows. That's been the rule since spring 2026 and can tighten or loosen without much notice. A capped synthetic lawn doesn't need to follow it at all."),
        faq("How close to Lake Hancock or the Peace River can turf go?",
            "Turf has to clear 10 feet of open ground before it reaches the water, whether that's Lake Hancock, the Peace River or a backyard retention pond, unless a seawall already stands between the two. Florida's 2026 rule sets that distance statewide, and separately bars turf from any ditch or swale a lot drains through."),
        ...
        faq("What should you ask the best artificial turf contractor near me in Bartow?",
...
_RES_S3 = ("Permits for a straightforward Bartow lawn",
           f"<p>A plain backyard conversion inside the city limit goes through Bartow's own Building Department at {ext(BUILD[1], '(863) 534-0157')}, a line staffed or voicemail-monitored around the clock, with the application filed online rather than mailed in. A search of Bartow's code doesn't turn up a section written for synthetic turf specifically, so {a('/laws/florida-hb-683/', "the 2026 state standard")} ends up doing most of the actual governing here; anything more specific to a given lot still comes down to a phone call rather than an assumption. A subdivision built under a homeowners' association layers its own review on top, and {a('/laws/hoa-rules/', "state law keeps that review from reaching a backyard no one can see from the street")}. Either way, the crew still caps any irrigation head that used to reach the converted area, since {src('dep-rule', "the state rule")} bars watering synthetic turf through an in-ground system.</p>")
