...
HUB = page(
    ...
    capsule(f"US-441 carries you into cattle country well before it reaches Kenansville, about {MI} miles southeast of our Kissimmee base and near the far edge of where we take jobs. Turf priced here matches the rest of Central Florida, {price('residential')} a square foot as tracked this September, though a well-and-septic ranch lot is a rarer call for us than a subdivision closer to town."),
    "".join([
        sec("Ranch country at the edge of where we work",
            f"<p>Everywhere else in Osceola County, we are working a subdivision lot with municipal water and a neighbor within shouting distance. Kenansville is different: an unincorporated ranching community strung along US-441, closer to Lake Kissimmee and the Kissimmee Prairie than to another rooftop. Lots here commonly run well past the one-acre ceiling the state's synthetic-turf standard actually covers, so a lot of what applies automatically on a quarter-acre Kissimmee yard, no local turf ban, a capped permeability number, simply is not in play on a bigger parcel out here. Jobs this far out are occasional rather than routine, and an install typically gets scheduled around other far-south Osceola stops rather than as a standalone trip.</p>"),
        table("Kenansville yard types and what we do differently",
              ["Property type", "What it looks like", "How the job changes"],
              [["Cattle-ranch homesteads", "House and yard on a multi-acre working parcel", "Often over the state rule's one-acre ceiling, so only Osceola County's own code, silent on turf, applies"],
               ["Lake Marian and canal fish camps", "Waterfront lots along Lake Marian or a connecting canal", "Same 10-ft state shoreline rule as anywhere; nothing closer without a seawall in place"],
        ...
        sec("Permits: Osceola County, not a city hall",
            f"<p>Kenansville has no city government, so every permit question runs through the Osceola County Building Department at 407-742-0200, filed through the county's {ext(OSCEOLA_PERMITS[1], 'Online Permit Center')}. The county's Land Development Code does not mention synthetic turf, the silence that applies countywide, so {a('/laws/permits/osceola-county/', 'our Osceola County permit page')} covers this address the same as one inside Kissimmee. A parcel search on the {ext(OSCEOLA_PA[1], 'Osceola County Property Appraiser’s site')} confirms boundaries and acreage first, and acreage is the number that matters most: {a('/laws/florida-hb-683/', 'the state’s May 2026 turf standard')} only reaches a lot of one acre or less, so a bigger ranch parcel answers to the county's ordinary code alone. We found no CDD or HOA covering this part of the county, though {a('/laws/hoa-rules/', 'Florida’s association visibility statute')} would still apply to any property that does carry one.</p>"),
        sec("No city water out here: wells, septic and a state rule that follows",
            f"<p>{ext(TOHO_AREA[1], 'Toho Water Authority’s own service-area description')} centers on Kissimmee, St. Cloud, Poinciana and the unincorporated land close to them; we found no utility that lists Kenansville as served territory. A home out here typically pumps its own water from a private well and treats wastewater with a septic system, permitted through the {ext(DOH_OSCEOLA[1], 'Florida Department of Health in Osceola County')}, whose onsite-sewage program moved under FDEP on July 1, 2021 ({ext(OSCEOLA_SEPTIC[1], 'the county’s septic and well permit checklist')}). That changes the checklist more than a municipal lot ever would: keeping a septic tank's pump-out lid reachable, which the state standard requires, matters on nearly every job here rather than the rare Kissimmee lot never connected to Toho.</p>"),
        sec("Lakes, prairie and a river basin, not a subdivision pond",
            f"<p>Kenansville sits inside the South Florida Water Management District's Kissimmee Basin, the same broad district that drains the chain of lakes south from Kissimmee toward the Kissimmee River and, eventually, Lake Okeechobee ({ext(SFWMD_KISS[1], 'SFWMD’s Kissimmee River overview')}). Lake Marian runs to the edge of town, and Lake Kissimmee and Lake Jackson sit a short drive away inside the {ext(FWC_3LAKES[1], 'nearly 64,000-acre Three Lakes Wildlife Management Area')}, one of the largest stretches of dry prairie left in Florida; any turf near that shoreline still clears the state's 10-foot line, a distance only a seawall closes. The ground is flatwoods and prairie country: the Felda series common here is poorly to very poorly drained, a soil {ext(FELDA_OSD[1], 'USDA’s official series description')} puts within a foot of the surface for two to six months of the year, wetter than the sand under most Kissimmee-area lots.</p>"),
        sec("A cattle-town history, and no Census count of its own",
            f"<p>Kenansville grew up around the Okeechobee spur of the Florida East Coast Railway and took its name in 1914, honoring Mary Lily Kenan, Henry Flagler's third wife. The building known today as the Heartbreak Hotel opened in 1915 as the Piney Woods Inn and was renamed in 1955 ({ext(FL_MEMORY[1], 'Florida Memory’s photographic record of the building')}); the railroad closed in the 1950s and the Turnpike later routed traffic away from US-441, leaving a small, working ranching community rather than a town that grew into a city. The Census Bureau has never designated Kenansville its own place, so unlike Mount Dora or St. Cloud it carries no population count of its own; anyone here searching “best artificial turf installer near me in Kenansville” is mostly checking whether a Kissimmee-based crew still makes the drive, and we do.</p>"),
        "<!--AUTO:city-services-->",
    ]),
    faqs=[
...
LOCAL = {
    "residential": {
        ...
        "lede": capsule(f"A Kenansville yard prices no differently than one back in Kissimmee, {price('residential')} a square foot, checked this September. What changes is everything around the price: acreage instead of a platted subdivision, a well and a septic tank instead of city utilities, and a water table that sits closer to the surface than it does on higher ground near town."),
        "sections": [
            ("When the state's one-acre rule does not reach the property",
             f"<p>Florida's synthetic-turf standard only preempts local rules on a single-family lot of one acre or less. A working cattle property out here routinely runs well past that, which means the state's floor, no local ban, a capped permeability number, a guaranteed 10-foot waterbody setback, simply does not extend to the yard around the house the way it would on a smaller platted lot. In practice that changes less than it sounds like, since {a('/laws/permits/osceola-county/', 'Osceola County’s own Land Development Code')} does not mention synthetic turf either way, acreage or not. It is still worth knowing before assuming a rule from a Kissimmee job carries over automatically to a Kenansville ranch without checking the acreage first.</p>"),
            ...
            ("A higher water table than the sand up in town",
             f"<p>{ext(FELDA_OSD[1], 'The Felda series')} common to this stretch of flatwoods and prairie country is poorly to very poorly drained, with a water table that can sit within a foot of the surface for two to six months most years, wetter than the sand under a typical Kissimmee subdivision lot. That is one reason a base out here typically goes in near four inches of washed rock rather than two: a thinner layer barely lifts turf above ground that is already wet for a good part of the summer. Compared with the citrus-ridge sand under a {city('harmony')} or Lake County lot, which drains almost too well, a Kenansville yard is the opposite problem to solve.</p>"),
        ],
    ...
    "pet": {
        ...
        "lede": capsule(f"A ranch dog usually already has the run of the property, so what pet turf adds near Kenansville is one contained, mud-free, easy-to-rinse area close to the house rather than a whole acreage to keep clean. The rate is unchanged from the rest of our area: {price('pet')} a square foot, confirmed this September."),
        "sections": [
            ...
            ("More room to route around the septic drainfield",
             f"<p>Planning a dog run's fence line around a septic drainfield and pump-out lid is a step on nearly every rural Osceola County job, but a multi-acre Kenansville property usually gives us more options for where the run can sit relative to that infrastructure than a quarter-acre lot ever would. Instead of squeezing a run's footprint into whatever space is left after the drainfield, house and driveway are accounted for, there is often room to simply place the run somewhere else entirely and leave a wide, unambiguous buffer around the septic system, which is the simpler way to satisfy the state's pump-out-access requirement. That extra room also means a run rarely has to compromise on shape the way a tightly fenced suburban lot sometimes does, since there is usually space to keep it a simple rectangle rather than notching around a fixed obstacle.</p>"),
            ("Fencing a run near the Three Lakes boundary",
             f"<p>Properties toward the edge of town, closer to the Three Lakes Wildlife Management Area, back up to conservation land rather than another homeowner's fence. A dog run's perimeter fencing matters more here for keeping a pet contained near that boundary than it does on a lot backing onto a neighbor's yard, since there is no second fence on the other side doing any of that work. That is a fencing and layout conversation more than a turf one, but it shapes where a run's gate and boundary end up before base material ever gets ordered. A run built right along that edge also benefits from a solid, sight-blocking panel rather than open wire, since a dog that can see into the preserve tends to test the fence line harder than one facing an ordinary backyard.</p>"),
        ],
        "scenario": ("Sizing a run behind a house off Canoe Creek Road",
                     f"<p>Say you have a 300 sq ft area behind a ranch house off Canoe Creek Road, fenced for two dogs that otherwise have the run of a fenced pasture. At {price('pet')} a square foot, that run prices between $3,000 and $5,400 installed, covering the deeper base, fully permeable backing and odor-control infill a pet system needs. With acreage to work with, the fence line usually gets planned to leave a wide, unambiguous gap around the septic drainfield rather than squeezing the run's footprint against it the way a smaller in-town lot might have to, and the water for the rinse hose typically comes from the same well that supplies the house.</p>"),
        "faqs": [
    ...
    "putting": {
        ...
        "faqs": [
            ...
            faq("Does a lakefront lot near Lake Marian limit where a green can go?",
                "Yes, the same way any Florida lake works. Turf, a green included, has to stay 10 feet clear of the water's edge unless a seawall is already doing that separating, so the buildable area begins at that line rather than at the property boundary."),
        ],
        "sources": SRC,
    },
}
