...
SRC = [MASCOTTE_BUILDING, MASCOTTE_PORTAL, MASCOTTE_WATER, SJRWMD_MASCOTTE, SJRWMD_RESTRICT, IMMOKALEE_OSD, MYAKKA_OSD,
       SJRWMD_SOILMAP, MASCOTTE_POP, SUNSET_LAKE_SRC, LAKE_LINDA_SRC, SUNSET_LAKES_ESTATES_SRC, WOODBURY_SRC,
...
HUB = page(
    ...
    capsule(f"Synthetic turf installed in Mascotte runs {price('residential')} per square foot as of September 2026, the same Central Florida range as anywhere else we work. The city has grown faster than 96 percent of similarly sized U.S. cities since 2000, and most of that growth shows up as new subdivisions on flat, low-lying ground rather than the ridge terrain just up the road."),
    "".join([
...
LOCAL = {
    "residential": {
        ...
        "lede": capsule(f"Artificial grass installed in Mascotte costs {price('residential')} per square foot as of September 2026, matching the Central Florida range everywhere in our service area. Much of the current demand traces back to the city's new subdivisions, built on level flatwoods ground that behaves nothing like the sand ridge a few miles toward Groveland and Clermont."),
        "sections": [
            ("Flatwoods ground instead of ridge sand",
             "<p>Lake County's own general soil map groups the low ground around Mascotte with the Myakka and Immokalee flatwoods family rather than the excessively drained Candler and Astatula sands that dominate the ridge toward Groveland. Both series carry a shallow, seasonally high water table, closer to a couple of feet down than the 80-plus inches the ridge soils allow, since flatwoods ground was never as well drained to begin with.</p>"
    ...
    "pet": {
        ...
        "lede": capsule(f"Pet turf in Mascotte runs {price('pet')} per square foot as of September 2026. A dog run here is as likely to sit on a well-and-septic acreage lot as it is in a starter-home side yard barely wide enough for a wheelbarrow, and each layout answers to a different rule first.</p>".replace("</p>", "")),
        "sections": [
    ...
    "putting": {
        ...
        "lede": capsule(f"A backyard putting green in Mascotte costs {price('putting')} per square foot as of September 2026. Unlike the naturally rolling ridge lots toward Groveland, most Mascotte yards start dead level, which puts more of a green's contour on the crew's shaping plan rather than on ground that already leans one way.</p>".replace("</p>", "")),
        "sections": [
            ("Building contour where the ground starts flat",
             "<p>A putting green needs rolls, tiers and a slightly raised collar around each cup to read as real turf rather than a tabletop, and on Mascotte's flatwoods lots almost all of that shape has to be built rather than borrowed from the site. That's the reverse of a ridge lot near Groveland, where some grade change already exists before a shovel goes in.</p>"
