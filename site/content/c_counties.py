...
def lake():
    body = "".join([
        ...
        sec("Two water districts, one Lake County yard",
            f"<p>Lake County is genuinely split, and it's worth knowing which side a lot falls on before assuming a watering schedule. Roughly the southern and western edge of the county, Minneola, Groveland, Mascotte, Montverde and the southwest corner of Clermont, sits inside the Southwest Florida Water Management District's Green Swamp area, a small but distinct slice of the county's total footprint ({ext(LAKE_PLAN[1], 'Lake County’s own water supply plan')}; {ext(GREEN_SWAMP[1], 'Florida DEP’s Green Swamp overview')}). The rest of the county, including Mount Dora and most of Clermont, answers to the St. Johns River Water Management District instead, which runs a seasonal odd/even schedule and was under a tightened Phase III shortage order in 2026 ({ext(SJRWMD_RESTRICT[1], 'SJRWMD’s current restrictions')}).</p>"
