# -*- coding: utf-8 -*-
"""Create and deploy the kissimmeeturf-voice Twilio Serverless service for kissimmeeartificialturf.com.

Mirrors the ocoee-voice and windermere-voice services already on the account:

  caller dials (689) 202-3710
    -> /incoming   screens robocalls: "Welcome to Kissimmee Artificial Turf. This call may be recorded. Press any key to speak with our team."
    -> dials FORWARD_TO with a whisper on the owner's leg
    -> /whisper    "New lead from Kissimmee Artificial Turf. Press any key to accept."
...
UNIQUE = "kissimmeeturf-voice"
FRIENDLY = "Kissimmee Artificial Turf Call Flow"
NUMBER = "+16892023710"
FORWARD_TO = os.environ.get("FORWARD_TO", "+16892427487")  # confirm with the owner; read ocoee-voice's variable first
DEPENDENCIES = [
...
FUNCTIONS = {
  ...
  s1.prosody({ rate: '85%' }, 'Welcome to Kissimmee Artificial Turf. This call may be recorded. Press any key to speak with our team.');
  gather.pause({ length: 1 });
  const s2 = gather.say({ voice: 'Polly.Matthew-Neural' });
  s2.prosody({ rate: '85%' }, 'Welcome to Kissimmee Artificial Turf. This call may be recorded. Press any key to speak with our team.');
  twiml.hangup();
  return callback(null, twiml);
};
''',
  ...
  const s1 = gather.say({ voice: 'Polly.Matthew-Neural' });
  s1.prosody({ rate: '85%' }, 'New lead from Kissimmee Artificial Turf. Press any key to accept.');
  gather.pause({ length: 1 });
  const s2 = gather.say({ voice: 'Polly.Matthew-Neural' });
  s2.prosody({ rate: '85%' }, 'New lead from Kissimmee Artificial Turf. Press any key to accept.');
...
''',
  ...
  twiml.say(
    { voice: 'Polly.Matthew-Neural' },
    'Sorry we missed your call. Please leave your name, phone number, and a few details about your turf project after the beep, and we will get back to you as soon as possible.'
  );
...
def main():
    ...
    upd = req("POST", f"https://api.twilio.com/2010-04-01/Accounts/{SID}/IncomingPhoneNumbers/{num['sid']}.json", {
        "FriendlyName": "Kissimmee Artificial Turf Main Line (689) 202-3710",
        "VoiceUrl": voice_url,
        "VoiceMethod": "POST",
